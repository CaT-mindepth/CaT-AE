...
def solve_ILP(pkt_fields_def, tmp_fields_def, stateful_var_def, 
    ...
    num_of_entries_per_table, num_of_alus_per_stage, num_of_table_per_stage, num_of_stages,
    opt = True):
...
'ipv6_multicast_route_star_g':{
    ...
    num_of_alus_per_stage = num_of_table_per_stage
    num_of_stages = int(sys.argv[3])
    solver_flag = sys.argv[4]
    assert solver_flag == "Optimal" or solver_flag == "Feasible", "wrong objective type (either Optimal or Feasible)"
    if solver_flag == "Optimal":
        opt = True
    else:
        opt = False
